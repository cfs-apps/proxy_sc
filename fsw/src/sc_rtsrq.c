...
void SC_AutoStartRts(uint16 RtsNumber)
{
    ...
    if ((RtsNumber > 0) && (RtsNumber <= SC_NUMBER_OF_RTS))
    {
         ...
         */
        //bc CFE_MSG_Init(&CmdPkt.CmdHeader.Msg, CFE_SB_ValueToMsgId(SC_CMD_MID), sizeof(SC_RtsCmd_t));
        CFE_MSG_Init(CFE_MSG_PTR(CmdPkt.CmdHeader), CFE_SB_ValueToMsgId(SC_CMD_MID), sizeof(SC_RtsCmd_t)); //bc
        
        //bc CFE_MSG_SetFcnCode(&CmdPkt.CmdHeader.Msg, SC_START_RTS_CC);
        CFE_MSG_SetFcnCode(CFE_MSG_PTR(CmdPkt.CmdHeader), SC_START_RTS_CC); //bc
         ...
         */
        //bc CFE_SB_TransmitMsg(&CmdPkt.CmdHeader.Msg, true);
        CFE_SB_TransmitMsg(CFE_MSG_PTR(CmdPkt.CmdHeader), true); //bc
    }
    else
    {
